...
export class ProductDetailComponent implements OnInit {
  ...
  updateProduct() {
    ...
    let productToEdit: Product = {
      ...
      amountMountWarranty: this.formProductDetail.get('amountMountWarranty')
        .value,
    ...
    this.productService.updateProduct(id, productToEdit).subscribe(
      (data) => {
        this.alert.successAlet(
          `¡Producto Editado!`,
          `${productToEdit.name} editado Correctamente.`
        );
        this.noEditable();
      },
      (err) => {
      ...
      if (result.isConfirmed) {
        ...
        this.productService.deleteProdcut(id).subscribe(
          (data) => {
            this.alert.infoAlet(
              '¡Producto Eliminar!',
              `${this.product.name}, eliminado correctamente.`
            );
          },
          (err) => {
            this.alert.errorAlert(
              'Error :/',
              `Hay un error para eliminar el producto, ${this.product.name}.`
            );
          }
        );
